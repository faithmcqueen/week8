...
function pageLoaded(){
  ...
  var size=document.getElementById("hoodie-size");
  var quantity=document.getElementById("hoodie-quantity");
  var color = "White";
  var size;
  var style;
...
// Updates product description/title
  ...
  function descriptionUpdate () {
    description.innerHTML = size.value + " " + color + " Hoodie";
    imageUpdate();
    hiddenUpdate();
  }

  function imageUpdate () {
    image.src = "images/Hoodie-" + color +".png";
...
var OrderedColor = getParam('hoodie-color');
var OrderedSize = getParam('hoodie-size');
var OrderedAmt = getParam('hoodie-quantity');
...
var amtSection = document.getElementById('ordered-amt');
amtSection.innerHTML = "Quantity: <span>" + OrderedAmt + "</span></div>";

var orderedDescription = document.getElementById("ordered-description");
function orderSummary(){
  if (OrderedAmt > 1){
    orderedDescription.innerHTML = OrderedAmt + " " + OrderedSize + " " + OrderedColor + "Hoodies";
  }
  else {
    orderedDescription.innerHTML = OrderedSize + " " + OrderedColor + "Hoodie";
  }
}

var orderedShirt = document.getElementById("ordered-image");
orderedShirt.src="images/" + OrderedAmt + "-" + OrderedColor +".png";